...
let colors = {
	yellow: (msgs) => {
		return `<span style="color: #FFFF33">${msgs}</span>`;
	},
	green: (msgs) => {
		return `<span style="color: #ADFF2F">${msgs}</span>`;
	},
	cyan: (msgs) => {
		return `<span style="color: #00FFFF">${msgs}</span>`;
	}
};
...
proxy.on('proxyReq', (proxyReq, req, res, options) => {
	...
	if (REGEX_EXCEPT_DOMAINS.test(target.host)) {
		...
		process.send(`${colors.yellow(req.method)} ${colors.green(captureSite(req.url))} ${proxyReq.path} ${colors.cyan(req.connection.remoteAddress)}`);
	}
});
